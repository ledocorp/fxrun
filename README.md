...
fxrun reads a `Fxrun.toml` manifest, runs named recipes with dependencies, and can skip work when declared inputs are unchanged (content hash). Product logic is **fx**; a thin C host owns argv and process spawn.
...
```text
# Windows (PowerShell)
Invoke-WebRequest -Uri https://github.com/ledocorp/fxrun/releases/download/v0.1.1/fxrun-0.1.1-windows-x86_64.zip -OutFile fxrun.zip
Expand-Archive fxrun.zip -DestinationPath .
.\bin\windows\fxrun.exe --list

# Linux
curl -LO https://github.com/ledocorp/fxrun/releases/download/v0.1.1/fxrun-0.1.1-linux-x86_64.tar.gz
tar xzf fxrun-0.1.1-linux-x86_64.tar.gz
...
With `fx` 0.9.6+ on `PATH` and `FX_STD_ROOT` pointing at fx `std/`:

```text
fx build fxrun_lib.fx -o out --emit-c --host host/fxrun_host.c --no-guest \
  --link host/fxrun_spawn.c --link-include host --link-include <fx>/host/cli
```

(`--no-guest` must come after `--host`.)
